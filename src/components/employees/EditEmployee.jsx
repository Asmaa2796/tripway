...
const EditEmployee = () => {
  ...
  useEffect(() => {
    if (record) {
      setFormData((prev) => ({
        ...prev,
        name_ar: record?.basic_info?.name_ar || "",
        name_en: record?.basic_info?.name_en || "",
        email: record?.basic_info?.email || "",
        phone: record?.basic_info?.mobile || "",
        branch_id: record?.basic_info?.branch_id || null,
        customize_branch_id: Array.isArray(
          record?.customize_users?.customize_branches
        )
          ? record.customize_users.customize_branches.map((item) => ({
              value: item.id,
              label: item.name,
            }))
          : [],
        city_id: record?.basic_info?.city_id || null,
        status: record?.basic_info?.status == false ? 0 : 1,
        employment_number: record?.employment_data?.job_number || "",
        employment_date: record?.employment_data?.employment_date || "",
        stop_date: record?.employment_data?.stop_date || "",
        id_number: record?.employment_data?.identity || "",
        department_id: record?.employment_data?.management_id
          ? {
              value: Number(record.employment_data.management_id),
              label: record.employment_data.management_name,
            }
          : null,
        job_id: record?.employment_data?.job_id
          ? {
              value: Number(record.employment_data.job_id),
              label: record.employment_data.job_name,
            }
          : null,
        role_id: record?.basic_info?.authority_id || null,
        
        employment_image: record?.employment_data?.employment_image || null,
        company_source_id: Array.isArray(record?.supervisions?.company_sources)
          ? record.supervisions.company_sources.map((item) => item.id)
          : [],
        general_spervisor_id: Array.isArray(
          record?.supervisions?.general_supervisors
        )
          ? record.supervisions.general_supervisors.map((item) => item.id)
          : [],
        operation_spervisor_id: Array.isArray(
          record?.supervisions?.operation_supervisors
        )
          ? record.supervisions.operation_supervisors.map((item) => item.id)
          : [],
        company_application:
          record?.apps_access?.company_application == false ? 0 : 1,
        user_application:
          record?.apps_access?.user_application == false ? 0 : 1,
        owner_application:
          record?.apps_access?.owner_application == false ? 0 : 1,

        control_unregistered_companies:
          record?.customize_users?.control_unregistered_companies || null,
        company_control: record?.customize_users?.company_control || "",
      ...
      setSelectedAccount(
        record?.basic_info?.parent_id
          ? {
              label: record?.basic_info?.parent_name || "N/A",
              value: record?.basic_info?.parent_id,
            }
          : null
      );

      setSelectedRole(
        record?.basic_info?.authority_id
          ? {
              label: record?.basic_info?.authority_name || "N/A",
              value: record?.basic_info?.authority_id,
            }
          : null
      );

      setSelectedJob(
        record?.employment_data?.job_id
          ? {
              label: record?.employment_data?.job_name || "N/A",
              value: Number(record?.employment_data?.job_id),
            }
          : null
      );
      setSelectedManagement(
        record?.employment_data?.management_id
          ? {
              label: record?.employment_data?.management_name || "N/A",
              value: Number(record?.employment_data?.management_id),
            }
          : null
      );
      ...
      setSelectedCity(
        record?.basic_info?.city_id
          ? {
              label: record?.basic_info?.city_name || "N/A",
              value: record?.basic_info?.city_id,
            }
          : null
      );
      setSelectedCompanies(
        Array.isArray(record?.customize_users?.customize_branches)
          ? record.customize_users.customize_branches.map((item) => ({
              label: item.name || "N/A",
              value: item.id,
            }))
      ...
      setCompanyControlSelect(
        record?.customize_users?.company_control
          ? {
              label: t(`labels.${record.customize_users.company_control}`),
              value: record.customize_users.company_control,
            }
          : null
      );

      setWinchBranchControlSelect(
        record?.customize_users?.winch_branch_control
          ? {
              label: t(`labels.${record.customize_users.winch_branch_control}`),
              value: record.customize_users.winch_branch_control,
            }
          : null
      );

      setIndividualCustomerControl(
        record?.customize_users?.individual_customer_control
          ? {
              label: t(
                `labels.${record.customize_users.individual_customer_control}`
              ),
              value: record.customize_users.individual_customer_control,
            }
          : null
      );
      setUnregisteredCompaniesControl(
        record?.customize_users?.control_unregistered_companies
          ? {
              label: t(
                `labels.${record.customize_users.control_unregistered_companies}`
              ),
              value: record.customize_users.control_unregistered_companies,
            }
          : null
      );

      setCompanySource(
        Array.isArray(record?.supervisions?.company_sources)
          ? record.supervisions.company_sources.map((item) => ({
              label: item.name || "N/A",
              value: item.id,
            }))
          : []
      ...
      setGeneralSupervisor(
        Array.isArray(record?.supervisions?.general_supervisors)
          ? record.supervisions.general_supervisors.map((item) => ({
              label: item.name || "N/A",
              value: item.id,
            }))
          : []
      ...
      setOperationsSupervisor(
        Array.isArray(record?.supervisions?.operation_supervisors)
          ? record.supervisions.operation_supervisors.map((item) => ({
              label: item.name || "N/A",
              value: item.id,
            }))
          : []
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateEmployees({ id, data }));
  };
  ...
  useEffect(() => {
    ...
    //   console.log("Full error:", errorMessage);
      ...
      if (typeof errorMessage === "string") {
        ...
        translatedMessage = t("labels.failedToUpdate");
      }
