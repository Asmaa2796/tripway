...
const AddCarDepartments = () => {
  ...
  useEffect(() => {
    setTitle(`${t("sidenav.car_departments")} > ${t("labels.edit")}`);
    document.title = `${t("sidenav.car_departments")} > ${t("labels.edit")}`;
    dispatch(carDepartmentRecord(id));
     return () => {
  ...
  if (record) {
    setFormData({
      ...
      sub_departments: record?.sub_departments?.map((sub) => ({
        ...sub,
        status: sub.status === "1" ? 1 : 0,
      })) || []
    });
  }
}, [record]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(clearState());
    dispatch(updatecarDepartment({id,formData}));
  };
