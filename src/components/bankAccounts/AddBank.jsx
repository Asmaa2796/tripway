...
import { addBank, clearState } from "../../redux/Slices/BanksSlice";
...
const AddBank = () => {
  ...
  const { isLoading, error, success } = useSelector((state) => state.banks); 

  useEffect(() => {
    setTitle(`${t("sidenav.banks")} > ${t("btns.add")}`);
    document.title = `${t("sidenav.banks")} > ${t("btns.add")}`;
    return () => {
