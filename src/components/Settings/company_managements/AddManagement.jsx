...
const AddManagement = () => {
  ...
  useEffect(() => {
    setTitle(
      `${t("sidenav.companyManagements")} > ${t("labels.addManagement")}`
    );
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(clearState());
    dispatch(addManagement(formData));
    setFormData({
      name_ar: "",
      name_en: "",
    });
  ...
  useEffect(() => {
    if (success) {
      toast.success(t("labels.addedSuccessfully"), {
        onClose: () => {
          dispatch({ type: "company_managements/clearState" }); // Clear after toast
    ...
    if (error) {
      toast.error(t("labels.failedToAdd"), {
        onClose: () => dispatch({ type: "company_managements/clearState" }),
      });
    }
