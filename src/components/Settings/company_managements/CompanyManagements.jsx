...
import {
  clearState,
  deleteManagement,
  fetchCompanyManagements,
} from "../../../redux/Slices/CompanyManagementsSlice";
import { toast } from "react-toastify";
import TableLoader from "../../../pages/TableLoader";

const CompanyManagements = () => {
  const { t, i18n } = useTranslation("global");
  const { setTitle } = useTitle();

  const dispatch = useDispatch();
  const { companyManagements, isLoading, error, success } = useSelector(
    (state) => state.company_managements
  );

  useEffect(() => {
    setTitle(t("sidenav.companyManagements"));
  ...
  const totalPages = Math.ceil(companyManagements.length / itemsPerPage);
  const paginatedData = companyManagements.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );
  return (
    <>
      <Link to="add_management" className="btn save text-white my-3 text-sm">
        {t("labels.addManagement")}
      </Link>
              ...
              <tbody>
                {paginatedData.map((tr) => (
                  <tr key={tr.id}>
                    ...
                    <td className="sub-text">{tr.jobs}</td>
                    <td className="d-flex justify-content-center">
                      <Link className="btn" to={`edit_managemnet/${tr.id}`}>
...
export default CompanyManagements;
