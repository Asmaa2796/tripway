...
const AddPurchaseReturnsAccounts = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(clearState());
    dispatch(addPurchaseReturnsAccounts(formData));
    setFormData({
      name_ar: "",
      name_en: "",
      status: "",
  ...
  useEffect(() => {
    if (success) {
      toast.success(t("labels.addedSuccessfully"), {
        onClose: () => {
          dispatch({ type: "purchase_returns_accounts/clearState" }); // Clear after toast
    ...
    if (error) {
      toast.error(t("labels.failedToAdd"), {
        onClose: () => dispatch({ type: "purchase_returns_accounts/clearState" }),
      });
    }
...
export default AddPurchaseReturnsAccounts;
