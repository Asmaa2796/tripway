...
const EditRelatedPartyAccounts = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    setTitle(t("sidenav.edit_related_party_accounts"));
    document.title = t("sidenav.edit_related_party_accounts");
    return () => {
  ...
  useEffect(() => {
    dispatch(RelatedPartyAccountsRecord(id));
  }, [dispatch, id]);
  ...
  useEffect(() => {
    if (success) {
      toast.success(t("labels.updatedSuccessfully"), {
        onClose: () => {
          dispatch(clearState());
          navigate("/related_party_accounts");
        },
      });
    }
...
export default EditRelatedPartyAccounts;
