...
const AddRelatedPartyAccounts = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(clearState());
    dispatch(addRelatedPartyAccounts(formData));
    setFormData({
      name_ar: "",
      name_en: "",
      status: "",
  ...
  useEffect(() => {
    if (success) {
      toast.success(t("labels.addedSuccessfully"), {
        onClose: () => {
          dispatch({ type: "related_party_accounts/clearState" }); // Clear after toast
    ...
    if (error) {
      toast.error(t("labels.failedToAdd"), {
        onClose: () => dispatch({ type: "related_party_accounts/clearState" }),
      });
    }
...
export default AddRelatedPartyAccounts;
