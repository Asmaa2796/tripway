...
import {
  clearState,
  deleteRecord,
  fetchRelatedPartyAccounts,
} from "../../../redux/Slices/RelatedPartyAccountsSlice";
import { toast } from "react-toastify";
import TableLoader from "../../../pages/TableLoader";

const RelatedPartyAccounts = () => {
  const { t, i18n } = useTranslation("global");
  const { setTitle } = useTitle();

  const dispatch = useDispatch();
  const { related_party_accounts, isLoading, error, success } = useSelector(
    (state) => state.related_party_accounts
  );
  ...
  useEffect(() => {
    dispatch(clearState());
    dispatch(fetchRelatedPartyAccounts());
  ...
  const totalPages = Math.ceil(related_party_accounts.length / itemsPerPage);
  const paginatedData = related_party_accounts.slice(
  ...
  return (
    <>
      <Link
        to="/add_related_party_accounts"
        className="btn save text-white my-3 text-sm"
      >
        {t("sidenav.add_related_party_accounts")}
      </Link>
      <div className="table_wrapper">
        {isLoading ? (
          <TableLoader />
        ) : related_party_accounts && related_party_accounts.length > 0 ? (
                ...
                {paginatedData.map((tr, index) => (
                  <tr key={tr.id}>
                    ...
                    <td className="d-flex justify-content-center">
                      <Link
                        className="btn"
                        to={`/edit_related_party_accounts/${tr.id}`}
                      >
...
export default RelatedPartyAccounts;
