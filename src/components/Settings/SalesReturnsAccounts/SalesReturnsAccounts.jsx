...
const SalesReturnsAccounts = () => {
  const { t, i18n } = useTranslation("global");
  const { setTitle } = useTitle();

  const dispatch = useDispatch();
  const { sales_returns_accounts, isLoading, error, success } = useSelector(
  ...
  const totalPages = Math.ceil(sales_returns_accounts.length / itemsPerPage);
  const paginatedData = sales_returns_accounts.slice(
  ...
  return (
    <>
      <Link
        to="/add_sales_returns_accounts"
        className="btn save text-white my-3 text-sm"
      >
        {t("sidenav.add_sales_returns_accounts")}
      </Link>
      <div className="table_wrapper">
        {isLoading ? (
          <TableLoader />
        ) : sales_returns_accounts && sales_returns_accounts.length > 0 ? (
                ...
                {paginatedData.map((tr, index) => (
                  <tr key={tr.id}>
                    ...
                    <td className="d-flex justify-content-center">
                      <Link
                        className="btn"
                        to={`/edit_sales_returns_accounts/${tr.id}`}
                      >
...
export default SalesReturnsAccounts;
