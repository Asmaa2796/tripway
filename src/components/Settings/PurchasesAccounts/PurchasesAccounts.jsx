...
const PurchasesAccounts = () => {
  const { t, i18n } = useTranslation("global");
  const { setTitle } = useTitle();

  const dispatch = useDispatch();
  const { purchases_accounts, isLoading, error, success } = useSelector(
    (state) => state.purchases_accounts
  );
  ...
  useEffect(() => {
    dispatch(clearState());
    dispatch(fetchPurchasesAccounts());
  ...
  const totalPages = Math.ceil(purchases_accounts.length / itemsPerPage);
  const paginatedData = purchases_accounts.slice(
  ...
  return (
    <>
      <Link
        to="/add_purchases_accounts"
        className="btn save text-white my-3 text-sm"
      >
        {t("sidenav.add_purchases_accounts")}
      </Link>
      <div className="table_wrapper">
        {isLoading ? (
          <TableLoader />
        ) : purchases_accounts && purchases_accounts.length > 0 ? (
                ...
                {paginatedData.map((tr, index) => (
                  <tr key={tr.id}>
                    ...
                    <td className="d-flex justify-content-center">
                      <Link
                        className="btn"
                        to={`/edit_purchases_accounts/${tr.id}`}
                      >
...
export default PurchasesAccounts;
