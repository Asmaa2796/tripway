...
const AddPurchasesAccounts = () => {
  ...
  useEffect(() => {
    setTitle(t("sidenav.add_purchases_accounts"));
    document.title = t("sidenav.add_purchases_accounts");
    return () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(clearState());
    dispatch(addPurchasesAccounts(formData));
    setFormData({
      name_ar: "",
      name_en: "",
      status: "",
  ...
  useEffect(() => {
    if (success) {
      toast.success(t("labels.addedSuccessfully"), {
        onClose: () => {
          dispatch({ type: "purchases_accounts/clearState" }); // Clear after toast
          navigate("/purchases_accounts");
        },
      });
    }
    ...
    if (error) {
      toast.error(t("labels.failedToAdd"), {
        onClose: () => dispatch({ type: "purchases_accounts/clearState" }),
      });
    }
...
export default AddPurchasesAccounts;
