...
import { addGoodTypes, clearState } from "../../../redux/Slices/GoodTypesSlice";
...
const AddGoodTypes = () => {
  ...
  const { isLoading, error, success } = useSelector((state) => state.goodTypes);
  useEffect(() => {
    setTitle(`${t("sidenav.goodTypes")} > ${t("btns.add")}`);
    document.title = `${t("sidenav.goodTypes")} > ${t("btns.add")}`;
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(clearState());
    dispatch(addGoodTypes(formData));
  };
  useEffect(() => {
    if (success) {
        ...
        <div className="row align-items-center">
          ...
          <div className="col-xl-6 col-lg-6 col-md-12 col-12">
            ...
            <Select
              ...
              onChange={(selected) =>
                setFormData((prev) => {
                  const newDeptId = selected ? Number(selected.value) : null;
                  return {
                    ...prev,
                    car_department_id: newDeptId,
                    // clear values if dept != 2
                    ...(newDeptId !== 2 && {
                      degree_from: "",
                      degree_to: "",
                      type: "",
