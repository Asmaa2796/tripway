...
const SalesAccounts = () => {
  ...
  const { sales_accounts, isLoading, error, success } = useSelector(
  ...
  const totalPages = Math.ceil(sales_accounts.length / itemsPerPage);
  const paginatedData = sales_accounts.slice(
  ...
  return (
    <>
      <Link
        to="/add_sales_account"
        className="btn save text-white my-3 text-sm"
      >
        {t("sidenav.addSalesAccount")}
      </Link>
      <div className="table_wrapper">
        {isLoading ? (
          <TableLoader />
        ) : sales_accounts && sales_accounts.length > 0 ? (
                ...
                {paginatedData.map((tr, index) => (
                  <tr key={tr.id}>
                    ...
                    <td className="d-flex justify-content-center">
                      <Link className="btn" to={`/edit_sales_account/${tr.id}`}>
