...
import {
  ...
  deleteLoadTypes,
  fetchLoadTypes,
} from "../../../redux/Slices/LoadTypesSlice";
...
const LoadTypes = () => {
 ...
 const { loadTypes, isLoading, success, error } = useSelector((state) => state.loadTypes);

  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5;
  const totalPages = Math.ceil(loadTypes?.data?.length / itemsPerPage);

  const paginatedData = loadTypes?.data?.slice(
 ...
 const handleDelete = (e, id) => {
  ...
  if (window.confirm(t("labels.confirmDelete"))) {
    dispatch(deleteLoadTypes(id));
  }
};
useEffect(() => {
  if (success) {
    toast.success(t("labels.deleteSuccess"));
    dispatch(fetchLoadTypes()); // refresh list
    dispatch(clearState());
  }
                ...
                {paginatedData.map((tr, index) => (
                  <tr key={tr.id || index}>
                    ...
                    <td className="d-flex justify-content-center">
                      <Link className="btn" to={`/edit_load_types/${tr.id}`}>
