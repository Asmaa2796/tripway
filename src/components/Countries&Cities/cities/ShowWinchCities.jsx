...
const ShowWinchCities = () => {
  const { t, i18n } = useTranslation("global");
  const { setTitle } = useTitle();

  const dispatch = useDispatch();
  const { cities, isLoading, error, success, pagination, winchCities } =
  ...
  useEffect(() => {
    dispatch(clearState());
    dispatch(fetchWinchCities(currentPage));
  ...
  return (
    <>
      <div className="table_wrapper">
        {isLoading ? (
          <TableLoader />
        ) : winchCities && winchCities.length > 0 ? (
...
export default ShowWinchCities;
